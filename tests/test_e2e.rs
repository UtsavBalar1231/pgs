...
fn scan_stage_status_commit_workflow() {
    ...
    let stage_output = run_pgs(dir.path(), &["stage", "feature.rs"]).success();
    ...
    let commit_output = run_pgs(dir.path(), &["commit", "-m", "feat: add println"]).success();
    ...
    let final_scan = run_pgs(dir.path(), &["scan"]);
...
fn text_mode_scan_stage_status_commit_workflow() {
    ...
    let scan_output = run_pgs_raw(dir.path(), &["scan"]).success();
    let scan_stdout = String::from_utf8(scan_output.get_output().stdout.clone()).unwrap();
    ...
    let stage_output = run_pgs_raw(dir.path(), &["stage", "feature.rs"]).success();
    let stage_stdout = String::from_utf8(stage_output.get_output().stdout.clone()).unwrap();
    ...
    let status_output = run_pgs_raw(dir.path(), &["status"]).success();
    let status_stdout = String::from_utf8(status_output.get_output().stdout.clone()).unwrap();
    ...
    let commit_output = run_pgs_raw(dir.path(), &["commit", "-m", "feat: add println"]).success();
    let commit_stdout = String::from_utf8(commit_output.get_output().stdout.clone()).unwrap();
    ...
    let final_scan = run_pgs_raw(dir.path(), &["scan"]).code(1);
...
fn stage_unstage_is_idempotent() {
    ...
    // Scan before staging
    let scan_before = run_pgs(dir.path(), &["scan", "--full"]).success();
    ...
    // Unstage
    run_pgs(dir.path(), &["unstage", "hello.txt"]).success();

    // Scan after stage+unstage — should show the same changes as before
    let scan_after = run_pgs(dir.path(), &["scan", "--full"]).success();
...
fn scan_stage_commit_untracked_file_workflow() {
    ...
    // 2. Stage by path
    let stage_output = run_pgs(dir.path(), &["stage", "new_feature.rs"]).success();
    let stage_stdout = String::from_utf8(stage_output.get_output().stdout.clone()).unwrap();
    let stage_json: serde_json::Value = serde_json::from_str(&stage_stdout).unwrap();
    assert_eq!(stage_json["status"], "ok");
    ...
    // 4. Commit
    let commit_output = run_pgs(dir.path(), &["commit", "-m", "feat: add new_feature"]).success();
    let commit_stdout = String::from_utf8(commit_output.get_output().stdout.clone()).unwrap();
    let commit_json: serde_json::Value = serde_json::from_str(&commit_stdout).unwrap();
    assert!(commit_json["commit_hash"].is_string());
