...
fn unstage_file_restores_to_head() {
    ...
    // Unstage the file
    let output = run_agstage(dir.path(), &["unstage", "hello.txt"]).success();
...
fn unstage_multiple_line_selections_same_file_reports_each_selection_item() {
    ...
    run_agstage(dir.path(), &["stage", "multi.txt"]).success();

    let output =
        run_agstage(dir.path(), &["unstage", "multi.txt:2-2", "multi.txt:12-12"]).success();
    let stdout = String::from_utf8(output.get_output().stdout.clone()).unwrap();
    let json: serde_json::Value = serde_json::from_str(&stdout).unwrap();
