...
- Structured JSON output on every MCP call via `structured_content` (`structured_tool_result` at `src/mcp/contract.rs:835`).
...
> **Always read from `structured_content`. The `content` field is a human summary — never parse it. If you find yourself proposing "add structured JSON to MCP output", you are re-inventing a feature that already ships (`define_tool_output!` at `src/mcp/contract.rs:304`, `structured_tool_result` at `src/mcp/contract.rs:835`). See §0 Capability Truth Table.**
